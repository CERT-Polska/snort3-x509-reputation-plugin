...
static uint32_t SSL_decode_handshake_v3(const uint8_t* pkt, int size,
    uint32_t cur_flags, uint32_t pkt_flags, string &all_certificates, int &all_data)
{
    ...
    while (size > 0)
    {
        ...
        case SSL_HS_CERT:
            ...
            for(int i=0; i<(hs_len+(int)SSL_HS_PAYLOAD_OFFSET) and i<((int)size+(int)SSL_HS_PAYLOAD_OFFSET); i++)
            {
                char b = cert_pkt[i];
                all_certificates+=b;
            }
...
static uint32_t SSL_decode_v3(const uint8_t* pkt, int size, uint32_t pkt_flags,
    uint8_t* alert_flags, uint16_t* partial_rec_len, int max_hb_len, string &all_certificates, int &all_data)
{
    SSL_record_t* record;
    uint32_t retval = 0;
    uint16_t reclen;
    ...
    while (size > 0)
    {
        ...
        case SSL_HANDSHAKE_REC:
            ...
            if (!(retval & SSL_CHANGE_CIPHER_FLAG))
            {
                ...
                retval |= SSL_decode_handshake_v3(pkt, hsize, retval, pkt_flags, all_certificates, all_data);
            }
            else if (ccs)
            {
    ...
    uint8_t* alert_flags, uint16_t* partial_rec_len, int max_hb_len, string &all_certificates, int &all_data)
{
    ...
    if (!( prev_flags & SSL_HS_SDONE_FLAG ))
    {
        ...
        if (size == 5)
            return SSL_decode_v3(pkt, size, pkt_flags, alert_flags, partial_rec_len, max_hb_len, all_certificates, all_data);
